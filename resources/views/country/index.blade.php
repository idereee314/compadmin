                        ...
                        <div class="card card-custom">
                            <div class="card-header flex-wrap py-5">
                                <div class="card-title">
                                    <h3 class="card-label">УЛС жагсаалт 
                                    <span class="d-block text-muted pt-2 font-size-sm">УЛС</span></h3>
                                </div>
                            </div>
                            <div class="card-body">
                                ...
                                <div class="accordion accordion-light accordion-light-borderless accordion-svg-toggle" id="search">
                                    <div class="card">
                                        <div class="card-header">
                                            <div class="card-title collapsed" data-toggle="collapse" data-target="#search-country">
                                        ...
                                        <div id="search-country" class="collapse" data-parent="#search">
                                            <div class="card-body">
                                                <!--begin: Search Form-->
                                                <form class="mb-10" id="country-search-form" method="POST">
                                                    <div class="row mb-6">
                                ...
                                <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <table class="table table-separate table-head-custom table-checkable dataTable no-footer dtr-inline" id="country_datatable" role="grid" aria-describedby="kt_datatable_info" style="width: 1235px;">
                                                <thead>
                                                    <tr role="row">
                                                        <th>No.</th>
...
$(document).ready(function() {
    countryTable = $("#country_datatable").DataTable({
        processing:     true,
        serverSide:     true,
        deferRender:    true,
        autoWidth:      true,
        ...
        ajax: {
            url: '{{route('country.data.list')}}',
            type: 'POST',
            data: function ( d ) {
                d.name = $('#country-search-form input[id="name"]').val();
            },
        },
        columns: [
            {
        ...
        buttons: [
        {
            ...
            className: "btn btn-light-danger font-weight-bolder mb-2 {{ SecurityHelper::checkPermission(@Config::get('permission.country'), Config::get('permission.editable')) ? '' : 'd-none' }}",
            action: function ( e, dt, node, config ) {
                $.get('{!! route('country.create') !!}', showAddModal);
            }
        }]
	});
    ...
    $("#kt_reset").click(function(e){
        ...
        $('.datatable-input').each(function() {
            $(this).val('');
            countryTable.column($(this).data('col-index')).search('', false, false);
        });
        countryTable.draw();
    });
...
//         $('#countryModal .modal-content').html(data);
//         $('.selectpicker').selectpicker();      

//         $('#create-country-form select[name=organization_id]').select2({
//             width: 'resolve',
...
//             $('#create-country-form input[name=name]').val(e.params.data.name);
//             $('#create-country-form input[name=name_en]').val(e.params.data.name_en);
//         });
...
//                         $('#countryModal').find("#close").trigger('click');
...
//         $('#countryModal .modal-content').empty();
//     });
// }

// function countryModal(data){
//     $('#countryModal').modal();
//         $('#countryModal').on('shown.bs.modal', function(){
//             $('#countryModal .modal-content').html(data);
//             $('.selectpicker').selectpicker();      

//             $('#edit-country-form select[name=new_organization_id]').select2({
//                 width: 'resolve',
//                 dropdownAutoWidth : true,
//                 dropdownParent: $('#countryModal'),
//                 placeholder: "-- {{ trans('display.general_select') }} --",
...
//                 }
//             }).on('select2:select', function(e){
//                 $('#edit-country-form input[name=name]').val(e.params.data.name);
//                 $('#edit-country-form input[name=name_en]').val(e.params.data.name_en);
//             });
...
// {
//     $.get('/country/' + id + '/edit', countryModal);
// }
