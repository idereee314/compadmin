                                    ...
                                    @foreach(collect($eventResult)->groupBy('gender_code') as $genderCode => $genderResults)
                                        <div class="col-xl-12">
                                            <div class="card card-custom gutter-b">
                                                <div class="card-header">
                                                    <div class="card-title">
                                                        <h3 class="card-label">{{ config("enums.gender_code")[$genderCode] }}</h3>
                                                    </div>
                                                </div>
                                                <div class="card-body">
                                                    @foreach($genderResults->groupBy('category_name') as $categoryName => $categoryResults)
                                                        @foreach($categoryResults->groupBy(fn($item) => $item->start_age . '-' . $item->end_age) as $age => $ages)
                                                            @php
                                                                [$startAge, $endAge] = explode('-', $age);
                                                            @endphp
                                                                    ...
                                                                    <div class="col-xl-6">
                                                                        <div class="card card-custom gutter-b">
                                                                            <div class="card-header">
                                                                                <div class="card-title text-center">
                                                                                    <h3 class="card-label">
                                                                                        <strong>{{ $categoryName }} | {{ config("enums.gender_code")[$genderCode] }} | ({{ $startAge }}-{{ $endAge }}) | {{ $bus }} | {{ $weight }}</strong>
                                                                                    </h3>
                                                                                </div>
                                                                            </div>
                                                                            <div class="card-body">
                                                                                                    ...
                                                                                                    <td class="text-center border-right">
                                                                                                        <strong>
                                                                                                            @if($result->place_number > 3)
                                                                                                                {{ $result->place_number }}
                                                                                                            @else
                                                                                                                <i class="{{ config("enums.event_award")[$result->place_number] }}"></i>
                                                                                                            @endif
                                                                                                        </strong>
                                                                                                    </td>
                                                                                                    <td class="text-center border-right">
                                                                                                        <div class="d-flex align-items-center">
                                                                                                            ...
                                                                                                            @endif
                                                                                                            <div class="ml-3">
                                                                                                                <span class="text-dark-75 line-height-sm d-block pb-3" style="white-space: nowrap;">
                                                ...
                                                @foreach(collect($ages)->groupBy('bus') as $bus => $belts)
                                                    @foreach(collect($belts)->groupBy('weight') as $weight => $weights)
                                                        @foreach(collect($weights)->groupBy('gender_code') as $genderCode => $genders)
                                                            @php
                                                                $hasResult = $genders->firstWhere('result', '!=', null); // result байгаа эсэх
                                                                $rowClass = $hasResult ? 'table-success' : '';
                                                                $athleteCount = count($genders);
                                                                $isFinished = $genders->first()->is_finished ?? false;
                                                                $medalGiven = $genders->firstWhere('award', '!=', null) ? '✓' : '✗';
                                                            @endphp
                                                            <tr class="{{ $rowClass }}">
                                                                ...
                                                                <td class="text-center border-right"><strong>{{ $athleteCount }}</strong></td>
                                                                <td class="text-center border-right">
                                                                    <input type="checkbox" disabled {{ $isFinished ? 'checked' : '' }}>
                                                                </td>
                                                                <td class="text-center border-right"><strong>{{ $medalGiven }}</strong></td>
                                                                <td class="text-center border-right">
                                                                    <a href="#" class="btn btn-sm btn-primary">Засах</a>
                                                                </td>
                                                            </tr>
                                                        @endforeach
                                                    @endforeach
