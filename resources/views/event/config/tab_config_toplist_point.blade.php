    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
                ...
                <span class="text-muted pt-2 font-size-sm d-block">Тэмцээнд оролцох боломжтой насны ангилал</span></h3>
            </div>
            <div class="card-toolbar">
                <!--begin::Button-->
                <a href="javascript:;" class="btn btn-primary font-weight-bolder" id="toplist-add-point">
            ...
            <table class="table table-separate table-head-custom dtr-inline">
                <thead>
                    <tr>
                        ...
                        <th class="min-w-200px text-center">{{trans('display.start_position')}}</th> 
                        <th class="min-w-100px text-center">{{trans('display.end_position')}}</th>
                        <th class="min-w-125px text-center">{{trans('display.general_point')}}</th>
                ...
                <tbody>
                    @foreach($eventToplistPoint as $point)
                    <tr>
                        <td class="text-center">{{++$loop->index}}</td>
                        <td class="text-center">{{$point->start_pos}}</td>
                        <td class="text-center">{{$point->end_pos}}</td>
                        <td class="text-center">{{$point->point}}</td>
                        <td class="text-center">{{$point->created_at}}</td>
                        <td class="text-center pr-0">
                            <a href="javascript:;" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3 edit-point" data-pointid="{{$point->id}}">
                            ...
                            <a href="javascript:;" class="btn btn-icon btn-light btn-hover-primary btn-sm delete-point" data-pointid="{{$point->id}}">
                                <span class="svg-icon svg-icon-md svg-icon-primary">
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                        <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                            <rect x="0" y="0" width="24" height="24" />
...
<script>
$("#toplist-add-point").on('click', function(){
    var eventId = $("#event_id").val();

	$.get('{!! route('event.toplist.point.create') !!}?eventId='+eventId, function( data ) {
		$('#eventEntryModal').modal();
		$('#eventEntryModal').on('shown.bs.modal', function(){
			$('#eventEntryModal .modal-content').html(data);

			$('#create-event-toplist-point-form').validate({
				ignore: [],
...
$(".edit-point").on('click', function(){
    var eventId = $("#event_id").val();
    var pointId = $(this).data("pointid");

    $.get('/event/toplist/point/'+pointId+'/edit?eventId=' + eventId, function( data ) {
		$('#eventEntryModal').modal();
		$('#eventEntryModal').on('shown.bs.modal', function(){
			$('#eventEntryModal .modal-content').html(data);
            
			$('#update-event-toplist-point-form').validate({
				ignore: [],
				highlight:function(element) {
					$(element).parents('.form-group').addClass('has-error has-feedback');
				},
...
$(".delete-point").on('click', function(){
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: '/event/toplist/point/'+pointId,
                type: 'DELETE',
                success: function (response) {
                    if(response.status == 'success')
                    {
