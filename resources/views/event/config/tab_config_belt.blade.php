<div class="tab-pane fade show active" id="kt_tab_pane_11_3" role="tabpanel" aria-labelledby="kt_tab_pane_11_3">
    ...
    <div class="form-group">
        <div class="float-left">
            <a href="#" class="btn btn-text-primary btn-light-primary font-weight-bold ml-2" id="entry-add-belt">{{trans('display.general_new')}}</a>
        </div>
        <div class="clearfix"></div>
    </div>
    <div class="table-responsive mb-20">
        @if($configBelsts->count() > 0)
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center pr-0">
                        <a href="#" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3" onclick="updateRecord({{$belt->id}})">
                        ...
                        <a href="#" class="btn btn-icon btn-light btn-hover-primary btn-sm" onclick="deleteRecord({{$belt->id}})">
                            <span class="svg-icon svg-icon-md svg-icon-primary">
...
<script>
$("#entry-add-belt").on('click', function(){
    var eventId = $("#event_id").val();

	$.get('{!! route('event.entry.belt.create') !!}?eventId='+eventId, function( data ) {
			...
			$('#create-event-entry-belt-form').validate({
				...
				submitHandler: function(form) {
					$.ajax({
						...
						type: form.method,
						data: $(form).serialize(),
                        //+'&event_id='+eventId+'&_token={{ csrf_token() }}',
						beforeSend: function() {
							$('#preloader').show();
						},
...
function updateRecord(id){
    var eventId = $("#event_id").val();
    console.log(eventId)
	$.get('/event/entry/belt/'+id+'/edit?eventId=' + eventId, function( data ) {
		$('#eventEntryModal').modal();
		$('#eventEntryModal').on('shown.bs.modal', function(){
			$('#eventEntryModal .modal-content').html(data);
