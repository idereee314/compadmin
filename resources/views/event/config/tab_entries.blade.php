<div class="tab-pane fade show active" id="kt_tab_pane_11_3" role="tabpanel" aria-labelledby="kt_tab_pane_11_3">
    ...
    <div class="form-group">
        <div class="float-left">
            <a href="#" class="btn btn-text-primary btn-light-primary font-weight-bold ml-2" id="entry-add">{{trans('display.general_new')}}</a>
        </div>
        ...
        <table class="table table-borderless table-vertical-center">
            <thead>
                <tr>
                    ...
                    <th class="p-0 min-w-200px text-left">{{trans('display.general_name')}}</th> 
                    <th class="p-0 min-w-100px text-left">{{trans('display.general_name_en')}}</th>
                    <th class="p-0 min-w-125px text-center">{{trans('display.human_gender_code')}}</th>
                    <th class="p-0 min-w-125px text-center">{{trans('display.entrance_fee')}}</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center pr-0">
                        <a href="#" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3" onclick="updateRecord({{$entry->id}})">
                        ...
                        <a href="#" class="btn btn-icon btn-light btn-hover-primary btn-sm" onclick="deleteRecord({{$entry->id}})">
                            <span class="svg-icon svg-icon-md svg-icon-primary">
...
$("#entry-add").on('click', function(){
	...
	$.get('{!! route('event.entry.create') !!}?eventId='+eventId, function( data ) {
			...
			$('#create-event-config-entries-form').validate({
				...
				submitHandler: function(form) {
					$.ajax({
                        url: form.action,
						type: form.method,
						data: $(form).serialize()+'&event_id='+eventId+'&_token={{ csrf_token() }}',
						beforeSend: function() {
							$('#preloader').show();
						},
...
function updateRecord(id){
	$.get('/event/entry/'+id+'/edit', function( data ) {
		$('#eventEntryModal').modal();
		$('#eventEntryModal').on('shown.bs.modal', function(){
			$('#eventEntryModal .modal-content').html(data);
