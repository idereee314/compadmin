<div class="tab-pane fade show active" id="kt_tab_pane_11_3" role="tabpanel" aria-labelledby="kt_tab_pane_11_3">
    ...
    <div class="form-group">
        <div class="float-left">
            <a href="#" class="btn btn-text-primary btn-light-primary font-weight-bold ml-2" id="entry-add-weight">{{trans('display.general_new')}}</a>
        </div>
        <div class="clearfix"></div>
    </div>
    <div class="table-responsive mb-20">
        @if($configWeights->count() > 0)
        <table class="table table-borderless table-vertical-center">
            <thead>
                <tr>
                    <th class="p-0 w-40px text-center">#</th>
                    <th class="p-0 min-w-200px text-left">Төрөл</th> 
                    <th class="p-0 min-w-200px text-left">Жин</th> 
                    <th class="p-0 min-w-100px text-left">Нас</th>
            ...
            <tbody>
                <?php $i = 1;?>
			    @foreach($configWeights as $weight)
                <tr>
                    ...
                    <td class="text-left">{{$weight->age->start_age}} {{$weight->age->end_age == null ? '+' : '=>'.$weight->age->end_age}}</td>
                    <td class="text-center">{{$weight->created_at}}</td>
                    <td class="text-center pr-0">
                        <a href="#" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3" onclick="updateRecord({{$weight->id}})">
                        ...
                        <a href="#" class="btn btn-icon btn-light btn-hover-primary btn-sm" onclick="deleteRecord({{$weight->id}})">
                            <span class="svg-icon svg-icon-md svg-icon-primary">
...
$("#entry-add-weight").on('click', function(){
    var eventId = $("#event_id").val();
        ...
        $.get('{!! route('event.entry.weight.create') !!}?eventId='+eventId, function( data ) {
            ...
            $('#create-event-entry-weight-form input[name=entry_id]').select2({data: ""});
...
function updateRecord(id){
    var eventId = $("#event_id").val();
	$.get('/event/entry/weight/'+id+'/edit?eventId=' + eventId, function( data ) {
		$('#eventEntryModal').modal();
		$('#eventEntryModal').on('shown.bs.modal', function(){
		$('#eventEntryModal .modal-content').html(data);

        $('#update-event-entry-weight-form select[name=entry_id]').select2({
            placeholder: "-- {{ trans('display.general_select') }} --"
        });
