...
<form class="form" method="POST" id="update-event-config-entries-form" action="{{ route('event.entry.update', $eventEntry->id) }}">
    ...
    <div class="card-body">
        ...
        <div class="form-group row">
            ...
            <div class="col-md-9 col-lg-6">
                <input type="number" class="form-control" value="{{$eventEntry->entrance_fee}}" autocomplete="off" name="entrance_fee"  data-rule-required="true" data-msg-required="{{ trans('messages.validation_field_required') }}"/>
                <div class="error-here"></div>
            </div>
        </div>
    </div>
