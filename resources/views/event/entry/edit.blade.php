<form class="form" method="POST" id="update-event-config-entries-form" action="{{ route('event.entry.update', $eventEntry->id) }}">
    ...
    <div class="card-body">
        ...
        <div class="form-group row">
            <label class="col-md-3 col-form-label text-right">{{trans('display.duration')}}: <span class="text-danger">*</span></label>
            <div class="col-md-9 col-lg-6">
                <input type="number" class="form-control" value="{{$eventEntry->duration}}" name="duration" min="1" step="1" data-rule-required="true" data-msg-required="{{ trans('messages.validation_field_required') }}"/>
                <div class="error-here"></div>
            </div>
        </div>
        <div class="form-group row">
