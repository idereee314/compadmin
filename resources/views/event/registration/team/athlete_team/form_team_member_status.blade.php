                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-9">
                        <select class="form-control selectpicker" id="status" name="status" data-rule-required="true" data-msg-required="{{ trans('messages.validation_field_required') }}">
                            <option value="{{ $teamAthlete->status }}" selected="selected">{{ @Config::get("enums.event_registration_status")[$teamAthlete->status] }}</option>
                            ...
                            <div class="input-group-prepend">
                                <span class="input-group-text">
                                    <label class="checkbox checkbox-inline checkbox-success">
                                        <input type="checkbox" name="payment_status" id="payment_status" value="1" {{ @$eventTeamRegistration->payment && @$eventTeamRegistration->payment->status ? 'checked=checked' : ''}}>
                                        <span></span>
                                    </label>
                                </span>
                            </div>
                            <input type="number" class="form-control" id="amount" name="amount" value="{{ @$eventTeamRegistration->payment->amount }}" min="0" max="{{ @$entryFees->max('entrance_fee')}}" data-rule-required="true" data-msg-required="{{ trans('messages.validation_field_required') }}"/>
                            {{-- empty(@$eventTeamRegistration->payment) || $eventTeamRegistration->payment->from_type == 'admin' ? '' : 'readonly' --}}
                        </div>
                    </div>
                </div>
            </div>
