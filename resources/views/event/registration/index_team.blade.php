            ...
            <div class="container">
                ...
                <div class="card card-custom">
                    <div class="card-body">
                        ...
                        <div class="accordion accordion-light accordion-light-borderless accordion-svg-toggle" id="search">
                            <div class="card">
                                ...
                                <div id="search-registration" class="collapse" data-parent="#search">
                                    <div class="card-body">
                                        <!--begin: Search Form-->
                                        <form class="mb-5" id="event-team-registration-search-form" method="POST">
                        ...
                        <!--begin: Datatable-->
                        <table class="table table-separate table-head-custom" id="event-team-registration-datatable" style="margin-top: 13px !important">
                            <thead>
                            <tr>
                                <th width="5%">No.</th>
                                <th width="20%">{{trans('display.comp_academy')}}</th>
                                <th width="20%">{{trans('display.general_team')}}</th>
                                <th width="5%">{{trans('display.comp_member')}}</th>
                                <!-- <th width="20%">{{trans('display.comp_athlete_age_average')}}</th> -->
...
$(document).ready(function() {
    eventTable = $("#event-team-registration-datatable").DataTable({
        processing:     true,
        serverSide:     true,
        //deferRender:    true,
        //autoWidth:      true,
        ...
        ajax: {
            url: '{{ route('event.team.registration.data.list') }}',
            type: 'POST',
            data: function ( d ) {
                var dateArr = {};
                $('#event-team-registration-search-form input[name^="search_date"]').map(function(){
        ...
        columns: [
            ...
            {data: 'academy_name'},
            {data: 'team_name'},
            // {data: 'athlete_average_age'},
            {data: 'teamathlete_count'},
            {data: 'entry_name', "defaultContent": ""},            
        ...
        buttons: [
            {
                ...
                action: function ( e, dt, node, config ) {
                    $.get('{!! route('event.team.registration.create') !!}?event_id='+$('#event-team-registration-search-form input[id="search_event"]').val(), showAddModal);
                }
            },
            {
                ...
                action: function ( e, dt, node, config ) {
                    window.open('{!! route('event.registration.print.mandat') !!}?'+$('#event-team-registration-search-form').serialize(), '_blank');
                }
            },
            {
                extend: 'excelHtml5',
    ...
    $('#event-team-registration-search-form').on('submit', function(e) {
        eventTable.draw();
        e.preventDefault();
    });
    ...
    $('#event-team-registration-datatable tbody').on( 'click', 'tr td button.btn-status', function () {
        ...
        $.get('registration/team/change/status?reg_id='+id, showStatusModal);
    });
    ...
    $('#event-team-registration-datatable tbody').on( 'click', 'tr td a.win-place', function () 
    {
        var id = $(this).data("registrationid");

        $.get('registration/create/award?event_reg_id='+id, function( data ) {
            $('#memberTeamModal').modal();
            $('#memberTeamModal').on('shown.bs.modal', function(){
                $('#memberTeamModal .modal-content').html(data);
                    ...
                    submitHandler: function(form) {
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                {
                                    $('#memberTeamModal').find("#close").trigger('click');
            ...
            $('#memberTeamModal').on('hidden.bs.modal', function(){
                $('#memberTeamModal .modal-content').empty();
            });
        });
    });

    $('#event-team-registration-datatable tbody').on( 'click', 'tr td a.delete', function () {
        var id = $(this).data("registrationid");
    ...
    $('#event-team-registration-search-form select[name=search_entry]').on('change', function(){
        ...
        $('#event-team-registration-search-form select[name=search_entry_age]').select2({
        ...
        $('#event-team-registration-search-form select[name=search_entry_belt]').select2({
            placeholder: "-- {{ trans('display.general_all') }} --",
            data: jsonDataBelt,
            id: 'id',
            closeOnSelect: true,
    ...
    $('#event-team-registration-search-form select[name=search_entry_age]').on('change', function(){
        var ageId = $(this).val();
        var jsonDataWeight;
        ...
        $('#event-team-registration-search-form select[name=search_entry_weight]').select2({
            placeholder: "-- {{ trans('display.general_all') }} --",
            data: jsonDataWeight,
            id: 'id',
            closeOnSelect: true,
    ...
    $(".btn-filter-status-count").on('click', function(){
        ...
        $('#event-team-registration-search-form select[name=search_status]').val(status);
        $('#event-team-registration-search-form').submit();
    });

    $(".btn-filter-amount").on('click', function(){
        var amount = $(this).data('amount');

        $('#event-team-registration-search-form select[name=search_status]').val('{{ @Config::get('smart.event_registration_status')['approved'] }}');
        $('#event-team-registration-search-form select[name=search_amount]').val(amount);
        $('#event-team-registration-search-form').submit();
    });
...
function showAddModal( data ) {
    ...
    $('#memberTeamModal').on('shown.bs.modal', function(){
        ...
        $('#create-event-registration-form select[name=member_id]').select2({
            ...
            dropdownParent: $('#memberTeamModal'),
            placeholder: "-- {{ trans('display.general_select') }} --",
...
function showCountModal(data){
    $('#teamAthleteModal').modal();
    $('#teamAthleteModal').on('shown.bs.modal', function(){
        $('#teamAthleteModal .modal-content').html(data);

        $(this).off('shown.bs.modal');
    });

    $('#teamAthleteModal').on('hidden.bs.modal', function(){
        $('#teamAthleteModal .modal-content').empty();
    });
}
</script>
@endsection
