...
<form class="form" method="POST" id="update-event-registration-form" action="{{ route('event.registration.update', $eventRegistration->id)}}">
    ...
    <div class="card-body">
        ...
        <div class="form-group row {{$eventRegistration->academy->is_other == 0 ? 'd-none' : ''}}" id="academy_name_other">
            ...
            <div class="col-md-9">
                <input class="form-control" id="academy_name" name="academy_name" disabled="{{$eventRegistration->academy->is_other == 0 ? 'disabled' : 'enabled'}}" value="{{$eventRegistration->academy_name}}" data-rule-required="true" data-msg-required="{{ trans('messages.validation_field_required') }}"/>
            </div>
        </div>
